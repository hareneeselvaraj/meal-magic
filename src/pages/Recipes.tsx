...
const Recipes = ({ onOpenRecipeForm }: RecipesProps) => {
  ...
  const tagColors: Record<string, string> = {
    ...
    'Sweet': 'bg-amber-100 text-amber-600',
    'Light': 'bg-sky-100 text-sky-600',
    'Balanced': 'bg-emerald-100 text-emerald-600',
    'Iron Rich': 'bg-rose-100 text-rose-600',
    'Vitamin Rich': 'bg-violet-100 text-violet-600',
  };
  ...
  const renderRecipeCard = (recipe: Recipe) => {
    ...
    return (
      <div key={recipe.id} className="rounded-2xl border bg-white/70 backdrop-blur-sm border-white/40 hover:border-emerald-200 transition-all duration-200 overflow-hidden">
        ...
        {isExpanded && (
          <div className="px-3.5 pb-3.5 space-y-3 border-t border-gray-100 pt-3 animate-fade-in">
            {recipe.healthTags.length > 0 && (
              <div className="flex gap-1.5 flex-wrap">
                {recipe.healthTags.map(ht => (
                  <span key={ht} className="text-[9px] px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 font-medium border border-emerald-200/50 flex items-center gap-0.5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {recipe.ingredients.map((item, i) => (
                  <div key={i} className="flex items-center gap-1.5 text-xs px-2 py-1.5 rounded-lg bg-gray-50">
            ...
            <div>
              <h5 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">📝 Steps</h5>
              <ol className="space-y-1.5">
                {recipe.instructions.map((step) => (
                  <li key={step.stepNumber} className="flex gap-2 text-xs text-gray-600">
                    <span className="shrink-0 w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-[10px] font-bold mt-0.5">{step.stepNumber}</span>
                    <div className="flex-1">
                      <span>{step.text}</span>
                      {step.durationMinutes && <span className="ml-1 text-[10px] text-gray-400">({step.durationMinutes}m)</span>}
                    </div>
                  </li>
                ))}
              </ol>
            ...
            {recipe.nutritionPer100g.calories > 0 && (
              <div>
                <h5 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">🥗 Nutrition (per 100g)</h5>
                <div className="grid grid-cols-4 gap-2">
                  {[
                    ...
                    <div key={n.l} className="text-center p-1.5 rounded-lg bg-emerald-50/50">
                      <div className="text-xs font-bold text-gray-700">{n.v}{n.u}</div>
                      <div className="text-[9px] text-gray-400">{n.l}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="space-y-1.5">
                  {recipe.videoLinks.map((vid, vi) => (
                    <a key={vi} href={vid.url} target="_blank" rel="noreferrer"
                      className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-gray-50 hover:bg-gray-100 transition-all">
                      ...
                      <span className={cn(
                        ...
                        vid.originalLanguage === 'en' ? 'bg-blue-100 text-blue-700' :
                        vid.originalLanguage === 'ta' ? 'bg-orange-100 text-orange-700' : 'bg-purple-100 text-purple-700'
                      )}>
    ...
    return (
      <div className="space-y-6 pb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          {activeCuisines.map(cuisine => {
            ...
            return (
              <div
                ...
                className={cn(
                  "group relative overflow-hidden rounded-3xl p-5 cursor-pointer bg-white/70 backdrop-blur-xl border border-white/80 shadow-sm transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:shadow-emerald-500/10 hover:bg-white/90",
                  isDeleting && "ring-2 ring-rose-400"
                )}
              >
                {/* Sweep Background Gradient */}
                <div className="absolute -top-10 -right-10 w-32 h-32 bg-gradient-to-br from-emerald-100/60 to-teal-50/20 blur-2xl rounded-full transition-transform duration-700 group-hover:scale-150" />
                
                <div className="relative z-10">
                  <div className="w-[52px] h-[52px] bg-white/60 backdrop-blur-md rounded-2xl shadow-sm border border-white flex items-center justify-center text-3xl mb-4 group-hover:-translate-y-1 group-hover:shadow-md group-hover:bg-white transition-all duration-500">
                    <span className="drop-shadow-sm group-hover:scale-110 transition-transform duration-500">{cuisine.emoji || '🍽️'}</span>
                  </div>
                  <h3 className="text-sm font-bold text-gray-900 group-hover:text-emerald-700 transition-colors">{cuisine.name}</h3>
                  <p className="text-[10px] font-medium text-gray-400 mt-0.5">{cuisine.nameInTamil}</p>
                  ...
                  {isDeleting ? (
                    <div className="flex items-center gap-2 mt-4" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => setDeleteConfirm(null)}
                        className="flex-1 py-2 rounded-xl bg-gray-100/80 text-gray-600 text-[11px] font-bold hover:bg-gray-200 active:scale-95 transition-colors">Cancel</button>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 mt-4" onClick={e => e.stopPropagation()}>
                      <button onClick={() => openEditCuisine(cuisine)}
                        className="w-8 h-8 rounded-full bg-gray-100/80 flex items-center justify-center hover:bg-emerald-100 hover:text-emerald-700 transition-all text-gray-500">
                        <Edit2 size={13} />
                      </button>
                      {!cuisine.isDefault && (
                        <button onClick={() => setDeleteConfirm(cuisine.id)}
                          className="w-8 h-8 rounded-full bg-gray-100/80 flex items-center justify-center hover:bg-rose-100 hover:text-rose-600 transition-all text-gray-500">
                          <Trash2 size={13} />
                        </button>
                      )}
                      {cuisine.isDefault && (
                        <span className="text-[9px] text-emerald-700 bg-emerald-100/80 px-2.5 py-1 rounded-full font-bold uppercase tracking-widest ml-auto">Default</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <button
            onClick={openAddCuisine}
            className="group rounded-3xl border-2 border-dashed border-gray-200/80 bg-gray-50/30 p-5 flex flex-col items-center justify-center gap-3 min-h-[180px] hover:border-emerald-300/80 hover:bg-emerald-50/40 hover:-translate-y-1 hover:shadow-lg transition-all duration-500"
          >
            <div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
              <Plus size={20} className="text-gray-400 group-hover:text-emerald-500 transition-colors" />
            </div>
            <span className="text-xs text-gray-500 font-bold group-hover:text-emerald-600 transition-colors">Add Cuisine</span>
          </button>
        </div>
        ...
        {showCuisineForm && (
          <div className="fixed inset-0 z-[97] bg-black/40 backdrop-blur-sm flex items-center justify-center px-4" onClick={() => setShowCuisineForm(false)}>
            <div className="bg-white rounded-2xl w-full max-w-sm p-5 animate-fade-in shadow-xl" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-bold text-gray-800">{editingCuisineId ? 'Edit Cuisine' : 'New Cuisine'}</h3>
                <button onClick={() => setShowCuisineForm(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center">
                  ...
                  {emojiOptions.map(e => (
                    <button key={e} onClick={() => setFormEmoji(e)}
                      className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-lg transition-all",
                        formEmoji === e ? "bg-emerald-100 ring-2 ring-emerald-400 scale-110" : "bg-gray-50 hover:bg-gray-100"
                      )}>{e}</button>
                  ))}
                </div>
              </div>
              ...
              <button onClick={handleCuisineSubmit} disabled={!formName.trim()}
                className={cn("w-full py-3 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2",
                  formName.trim() ? "bg-gradient-to-r from-emerald-500 to-teal-600 text-white shadow-lg" : "bg-gray-200 text-gray-400 cursor-not-allowed"
                )}>
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className="w-9 h-9 rounded-full bg-white/70 border border-white/40 flex items-center justify-center hover:bg-white transition-all"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/70 backdrop-blur-sm border border-white/40 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-300"
        />
        {searchQuery && (
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        <button onClick={() => setSelectedSlot(null)}
          className={cn("text-xs px-3 py-1.5 rounded-full font-medium border transition-all whitespace-nowrap shrink-0",
            !selectedSlot ? "bg-teal-600 text-white border-teal-600" : "bg-white/70 text-gray-500 border-gray-200"
          )}>All Meals</button>
        {MEAL_SLOT_CONFIG.map(s => (
          <button key={s.key} onClick={() => setSelectedSlot(selectedSlot === s.key ? null : s.key)}
            className={cn("text-xs px-3 py-1.5 rounded-full font-medium border transition-all whitespace-nowrap shrink-0",
              selectedSlot === s.key ? "bg-teal-600 text-white border-teal-600" : "bg-white/70 text-gray-500 border-gray-200"
            ...
            {videoProcessing ? (
              <div className="flex flex-col items-center py-8 gap-4">
                ...
                <div className="text-center">
                  <p className="text-sm font-bold text-gray-800">Processing Video...</p>
                  <p className="text-xs text-gray-400 mt-1">Extracting ingredients & steps</p>
                </div>
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="text-base font-bold text-gray-800">Add from Video</h3>
                      <p className="text-[10px] text-gray-400">Extract recipe from YouTube / Instagram</p>
                    </div>
                  </div>
                  <button onClick={() => setShowVideoImport(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center">
                    <X size={16} className="text-gray-500" />
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Video URL</label>
                    <input
                      type="url" value={videoUrl} onChange={e => setVideoUrl(e.target.value)}
                      placeholder="https://youtube.com/watch?v=... or Instagram link"
                      className="w-full px-3 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-300"
                      autoFocus
                    />
                    {videoUrl && (
                  ...
                  <div>
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Video Language</label>
      ...
      {pdfRecipe && (
        <div className="fixed inset-0 z-[97] bg-black/40 backdrop-blur-sm flex items-center justify-center px-4" onClick={() => setPdfRecipe(null)}>
          <div className="bg-white rounded-2xl w-full max-w-xs p-5 animate-fade-in shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="text-center mb-4">
              ...
              <h3 className="text-base font-bold text-gray-800">Download Recipe PDF</h3>
              <p className="text-xs text-gray-400 mt-1">{pdfRecipe.name}</p>
            </div>
            <p className="text-xs text-gray-500 font-medium text-center mb-3">Choose language:</p>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => handleDownloadPDF('en')}
                className="py-3 rounded-xl border-2 border-emerald-200 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-sm font-semibold transition-all flex flex-col items-center gap-1 active:scale-95">
                <span className="text-lg">🇬🇧</span>
                English
              </button>
              <button onClick={() => handleDownloadPDF('ta')}
                className="py-3 rounded-xl border-2 border-orange-200 bg-orange-50 hover:bg-orange-100 text-orange-700 text-sm font-semibold transition-all flex flex-col items-center gap-1 active:scale-95">
                <span className="text-lg">🇮🇳</span>
                தமிழ்
              </button>
            </div>
            <button onClick={() => setPdfRecipe(null)} className="w-full mt-3 py-2 text-xs text-gray-400 hover:text-gray-600 font-medium">Cancel</button>
          </div>
        </div>
      )}
    </div>
