...
interface NutriMomDB extends DBSchema {
...
let dbPromise: Promise<IDBPDatabase<NutriMomDB>> | null = null;

export function getDB() {
  if (!dbPromise) {
    dbPromise = openDB<NutriMomDB>('nutrimom', 1, {
      upgrade(db) {
...
export async function readAll<K extends StoreName>(store: K): Promise<NutriMomDB[K]['value'][]> {
  const db = await getDB();
  return db.getAll(store);
}

export async function writeAll<K extends StoreName>(store: K, values: NutriMomDB[K]['value'][]) {
  const db = await getDB();
